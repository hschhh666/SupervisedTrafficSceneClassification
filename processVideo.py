...
args.tarNpyFeatPath = args.video[:-4]+'_feat.npy'
args.tarNpyPredPath = args.video[:-4]+'_pred.npy'
# ===============读取视频及信息===============
...
memory_after_fc = torch.ones(frame_num, 4).cuda()
...
_, pred = memory_after_fc.topk(1, 1, True, True)
pred = pred.t().cpu()[0]
memory_after_fc = memory_after_fc.cpu().numpy()
np.save(args.tarNpyFeatPath, memory_after_fc)
np.save(args.tarNpyPredPath, pred)
cap.release()
out.release()
print('Save npy to %s. Done.'%args.tarNpyFeatPath)
print('Save npy to %s. Done.'%args.tarNpyPredPath)